...
class InteractiveSchematicCanvas:
    ...
    def add_or_update_block(
        ...
        # Auto position if none provided
        if x is None or y is None:
            idx = len(self.blocks)
            col = (idx % 3) * 20 + 2
            row = (idx // 3) * 6 + 2
            x = max(1, min(self.width - 16, col))
            y = max(1, min(self.height - 5, row))
        ...
        for ref, b in self.blocks.items():
            ...
            if 0 <= by < self.height and 0 <= bx < self.width:
                ...
                for x in range(bx, min(self.width, bx + bw)):
                    grid[by][x] = border_h
                grid[by][bx] = c_tl
                if bx + bw - 1 < self.width:
                    grid[by][bx + bw - 1] = c_tr

                # Bottom edge
                if by + bh - 1 < self.height:
                    for x in range(bx, min(self.width, bx + bw)):
                ...
                # Sides & Inner content
                for y in range(by + 1, min(self.height, by + bh - 1)):
                    grid[y][bx] = border_v
                    if bx + bw - 1 < self.width:
